FILTER_PROMPT = """
The following are one or more user requests from a conversation between a user and AI assistant (the assistant's responses are omitted). Each turn starts with the prefix "Human:". You are also given a description of an occupational task. Rewrite the user requests but remove all turns that are not related to the occupational task.
...
Answer with the rewritten user requests and nothing else.
""".strip()
...
- In two sentences, summarize what the user wants the assistant to do. Start this with: "The user wants the assistant to...". Write with a neutral tone and make the summary as detailed as possible (but focus on the task itself, rather than e.g., background information given). You may reference short quotes (a couple words at most) from the original requests if necessary. Ignore any part of the request that does not correspond to an occupational task (e.g., introductions like "what model are you?").
...
(C) Part or all of the request replaces a significant part (more than 33%) of a real-world task that falls under the Target responsibility.
...
CLASSIF_PROMPT_SKILLS = """
# Background
You are an economist looking to study how well AI assistants perform on occupational tasks. To do this, you want to build a benchmark. Specifically, you have a dataset of user requests made to AI assistants and you want to find samples that would accurately measure the performance of AI assistants on a target occupational responsibility.

# Instructions
Below is a target responsibility (which describes a category of related individual occupational tasks) and an occupation that you're studying (you should only consider the target responsibility as performed by the target occupation). Below is also a description of a request made to an AI assistant. Determine the amount of overlap between the skills required to perform the request and the skills required by a worker in the target occupation to perform the target responsibility.
...
Which of the following statements best describes the request in relation to the Target responsibility as performed by the Target occupation?
(A) The skills required by a worker in the Target occupation to perform the Target responsibility is pretty much or entirely the same as the skills required to perform the request.
(B) The skills required by a worker in the Target occupation to perform the Target responsibility is mostly (more than 80%) the same as the skills required to perform the request.
(C) The skills required by a worker in the Target occupation to perform the Target responsibility is generally (more than 50%) the same as the skills required to perform the request.
(D) The skills required by a worker in the Target occupation to perform the Target responsibility has a non-negligible (more than 10%) overlap with the skills required to perform the request.
(E) The skills required by a worker in the Target occupation to perform the Target responsibility has 10%% or less overlap with the skills required to perform the request.
(F) The request is topically related to the Target responsibility but has no overlap in skills OR the request is completely unrelated to the Target responsibility as performed by the Target occupation.

When answering this question, only consider the part of the request most relevant to the Target responsibility. You may ignore irrelevant parts of the request.

First, in at most four sentences carefully reason about the question in <thinking></thinking> tags. Then, put your answer in <answer></answer> tags (only put the letter here and nothing else). Make sure you follow this format exactly:
<thinking>In one word, the action the requester wants the assistant to perform is: [main requested action (e.g., summarization, programming, etc.) in a single word; do not under any circumstances include more than one word here.]. To carry out [target responsibility], a worker in [occupation] must be able to [all skills required for the responsibility]. [reasoning about the answer] Therefore, the answer is [repeat the answer that you choose, e.g., "The skills required ..."]"</thinking>
<answer>A, B, C, D, E, or F</answer>
""".strip()
