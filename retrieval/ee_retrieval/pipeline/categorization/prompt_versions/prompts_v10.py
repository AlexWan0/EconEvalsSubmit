...
DWA_CLASSF_PROMPT_OCC = """
# Background
You are an economist looking to study how well AI assistants perform on occupational tasks. To do this, you want to build a benchmark. Specifically, you have a dataset of user requests made to AI assistants and you want to find samples that would accurately measure the performance of AI assistants on a target occupational responsibility.

# Instructions
Below is a description of a request made to an AI assistant. Below is also a target responsibility (which describes a category of related individual occupational tasks) and an occupation that you're studying (you should only consider the target responsibility as performed by the target occupation). Determine the amount of overlap between the request and the target responsibility as performed by the target occupation.
...
1) Reread the description of the request above. Also, make sure to ignore requests that are completely irrelevant to the Target responsibility---you don't need to consider these when making your decision.
2) Reread the Target responsibility.
3) In <thinking></thinking> tags, reason about the answer in exactly four sentences. Make sure to only answer using only the information provided to you. Avoid speculation.
...
<thinking>
- The real-world task most relevant to the request that falls under [target responsibility] and also would be performed by a worker in "{category_occ}" is [describe most relevant task].
- The part of the request most relevant to this task is [describe most relevant part of request].
...
- Therefore, [repeat the answer that you choose, e.g., part or all of the request ..."]"</thinking>
<answer>A, B, C, D, E, or F</answer>
""".strip()

CLASSIF_PROMPT_OCC = """
# Background
You are an economist looking to study how AI assistants may assist or replace certain occupations. To do this, you want to build a benchmark. Specifically, you have a dataset of user requests made to AI assistants and you want to find samples that reflect responsibilities performed by certain occupations.

# Instructions
Below is an occupation that you're studying. Below is also a description of a request made to an AI assistant. Determine whether the request contains or would help with a task that would fall under the responsibilities of that occupation.
...
Additionally, you've already made some progress toward this task. You found a relevant occupational responsibility that is performed by someone working in the Target occupation ({category_occ}): {category_task_detailed}. However, many occupations may perform this task: **you need to determine the relevance of the request to the Target occupation specifically**.

Which of the following statements best describes the request in relation to the Target occupation?
(A) This request contains or would help with a task that would precisely fall under the responsibilities of someone working in the occupation of "{category_occ}".
(B) This request contains or would help with a task that would fall under the responsibilities of a related occupation, but not exactly of someone working in the occupation of "{category_occ}".
(C) This request does not contain and would not help with a task that would fall under the responsibilities of either someone working in the occupation of "{category_occ}" or a related occupation.
...
Make sure you follow this format exactly:
<thinking>
- An occupation that also may be relevant to the request is "[related occupation]" and would involve [describe the related occupation in more detail].
- The occupation "{category_occ}" involves [describe the Target occupation in more detail].
- The real-world task most relevant to the request that specifically falls under the responsibilities of a worker in this occupation is [describe most relevant task].
- Considering only the most relevant part of the request in relation to the most relevant real-world task, [reasoning about the answer].
- Therefore, [repeat the answer that you choose, i.e., this request ...]</thinking>
